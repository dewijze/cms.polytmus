...
define('SIZE_LIMIT',			1980); // max thumb width / height you ever want made by ANYONE! Set to zero to disable
define('FILE_LIMIT',			1000); // max images in cache
...
if (SIZE_LIMIT !== 0) {
    $maxWidth		= ($maxWidth > SIZE_LIMIT) ? SIZE_LIMIT : $maxWidth;
    $maxHeight		= ($maxHeight > SIZE_LIMIT) ? SIZE_LIMIT : $maxHeight;
}
...
// Keep the size of the cache folder within a limit
// Lets say a 1980 px thumb is maybe 1 Mb, so 1000 images is about a 1 Gb max.
...
if (!$isDirEmpty) {
    ...
    krsort($files);
    end($files);         // move the internal pointer to the end of the array
    $key = key($files);
    if ($files && count($files) > FILE_LIMIT){
        unlink($files[$key]);
    }
}
