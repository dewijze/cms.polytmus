...
function fieldSave(key,val){
    ...
    $.post('index.php', {fieldname: key, content: val, fullurl: url}).done(function(data){
        if(key == 'global_themeSelect'){location.reload(true);}
...
function browse(key){
    $.post('index.php', {file: key}).done(function(data){
        if(key == 'global_themeSelect'){location.reload(true);}
        else {$("#"+key).html(data);}
        changing = false;
    });
